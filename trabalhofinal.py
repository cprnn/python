...
class Conta: 
    ...
    def deposito(self,dep):
        if(dep>0):
            ...
            return "Depósito realizado com sucesso. Seu salto atual é de R$"+str(self.__saldo)
        else:
    ...
    def saque(self, valor):
        if(valor > 0):
            if((self.__saldo - valor)>=0):
                ...
                return "O valor"+str(valor)+"foi sacado de sua conta com sucesso! Seu saldo atual é de R$"+str(self.__saldo)
            else:
...
class Especial(Conta):
    ...
    def __init__(self, nome):
        super().__init__(nome)
        print("Atenção: Você pode sacar até R$500,00 acima de seu saldo atual.")
        self.__debito = 0.0
...
class Poupanca(Conta):
    ...
    def rendimento(self, rende):
        self.__saldo = self.__saldo + (self.__saldo * rende)
